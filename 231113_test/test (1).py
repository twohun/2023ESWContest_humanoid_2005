...
def Camera_distance_hole2():
    ...
    if grabbed:
        ...
        #prop_X = prop_X * 1.031 # calibration
        distance_y = Robot_height * tan(radians(V_angle) - atan(-tan(radians(H_angle/2)*(2*prop_A - 1)))) #physical height distance of target
        distance_x = distance_wx - distance_ax #(Robot_height / cos(radians(V_angle - H_angle/2))) * tan(radians(W_angle/2)) * prop_X # physical height distance of target
        L1_norm = sqrt(np.square(distance_x) + np.square(distance_y))
        ...
        #cv2.drawContours(frame, line[1], -1, (255,255,255), 3)
        cv2.imshow('visual', frame)
    #print('hole cx {0} hole cy{1}'.format(cX, cY))
        return cX, cY, distance_x, distance_y, L1_norm
    else:
    ...
    while True:
        ...
        if set_num == 1: # 공 찾기 
           ...
           # else:
            ...
            print("c_x : ", c_x, "c_y : ", c_y, "dist_x : ", dist_y)
            if set_num_4_cnt == 0: #홀 보일 때 안 보일때에 맞게 넘기는 부분
            ...
            if set_num_4_cnt == 1:
                ...
                for i in range(0,3) : # 20 40 60
                    ...
                    if (c_x != 0 or c_y != 0): # 인식하면
                        ...
                        set_num_4_cnt = 2
            '''if set_num_3_cnt == 0: #두연 코드
                TX_data_py2(serial_port, 73) #78도 돌기
                time.sleep(1)
                c_x, c_y, dist_x, dist_y, dist_l = Camera_distance_hole()
                set_num_3_cnt == 1
            elif set_num_3_cnt == 1: # 동심원 돌기
                rx_num = RX_data(serial_port)
                TX_data_py2(serial_port, 43)
                time.sleep(2)
                c_x, c_y, dist_x, dist_y, dist_l = Camera_distance_hole()
                set_num_3_cnt = 2
            elif set_num_3_cnt == 2: # 20, 40, 60 각도 체크
                head_val = [20, 30, 31]
                head_lis = [39, 37, 31]
    
                for i in range(len(head_val)) : # 20 40 60
                    TX_data_py2(serial_port, head_lis[i])
                    time.sleep(1)
                    c_x, c_y, dist_x, dist_y, dist_l = Camera_distance_hole()
                    if (c_x != 0 or c_y != 0): # 인식하면
                        ...
                        V_angle = head_cnt
                        set_num_3_cnt = 3
                        TX_data_py2(serial_port,neck_list[neck_pointer]) #고개를 다시 원래대로 (공을 보게)
                        time.sleep(1)
                        TX_data_py2(serial_port,55)
                        time.sleep(1)
                        c_x, c_y, dist_x, dist_y, dist_l = Camera_distance_hole()
            ...
            elif set_num_3_cnt == 3: # 오른발 보정
                c_x, c_y, dist_x, dist_y, dist_l = Camera_distance_ball()
                set_ball_hit(c_x)
                time.sleep(1)
                if c_x <= 490 or c_x >=410 :
                    set_num_3_cnt = 4
                    TX_data_py2(serial_port, 64) #81도 돌기
                    time.sleep(1)
 

            elif set_num_3_cnt == 4: # 홀 17 cm 보정
                c_x, c_y, dist_x, dist_y, dist_l = Camera_distance_hole()
                if float(c_x) <= 18 and float(c_x) >= 16:
                    TX_data_py2(serial_port,neck_list[neck_pointer]) #고개를 다시 원래대로
                    time.sleep(1)
                    TX_data_py2(serial_port,55)
                    time.sleep(1)
                    set_num_3_cnt = 3
                    ...
                    if float(c_x)>18: #실제 거리 기준으로는 17
                        TX_data_py2(serial_port, 3) #Right turn 5
                    elif float(c_x)<16 and c_x!=0 : #여기도 15
                        TX_data_py2(serial_port, 1) #Left turn 5'''
                  
            if set_num_4_cnt == 2: # 동심원 도는 부분
                ...
                if count_43 == 0:
                    ...
                    count_43+=1
                    cant_receive=0
                    set_num_4_cnt = 2 #여기서 머리 위아래 / 머리 위아래로 작동 시키로 0번으로 와서 찍고 옮기고 하는 방식으로 생각하기
                    c_x, c_y, dist_x, dist_y, dist_l = Camera_distance_hole2()
            ...
            if set_num_5_cnt == 0:
                ...
                if int(dist_x)!=0:
                    ...
                    set_hole_hit(dist_x)
                    c_x, c_y, dist_x, dist_y, dist_l = Camera_distance_hole2()
                    if float(dist_x) < 18.0 and float(dist_x) > 15.0 and calibration_count < 3:
                        TX_data_py2(serial_port, 39) #머리 전방 20도 내리기
                        set_num_5_cnt = 1 #여기는 발 밑 공 세팅 
                        calibration_count += 1
                    elif float(dist_x) < 18.0 and float(dist_x) > 15.0 and calibration_count > 2: #고개 내릴 때 calibration_count 올라감
            ...
            # 여기에 고개 내리고 보정하는ㄴ걸 넣든 하고 거기에 set_num 7 부분
            # 여기서 발 밑 공 세팅까지 하기 
        elif set_num == 6: # 퍼팅
            if shut_cout == 0:
                rx_num = RX_data(serial_port)
